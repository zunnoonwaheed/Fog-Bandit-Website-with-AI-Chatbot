...
const TextCard = ({ t }: { t: Testimonial }) => (
  <div className="bg-background border border-border rounded-2xl p-4 md:p-8 shadow-sm flex flex-col min-h-[248px] md:min-h-[300px]">
    ...
    <QuoteOpen className="text-secondary mt-4 md:mt-6 w-[18px] h-[14px] md:w-[26px] md:h-[20px]" />
    <p className="text-muted-foreground text-[12px] md:text-[15px] leading-[1.6] mt-3 md:mt-4 md:flex-1">{t.quote}</p>
    <div className="flex justify-end mt-2 md:mt-4">
      <QuoteClose className="text-secondary w-[18px] h-[14px] md:w-[26px] md:h-[20px]" />
    </div>
  </div>
);
  ...
  <div className={`relative rounded-2xl overflow-hidden w-full h-[420px] shadow-lg ${className}`}>
    ...
    <div className="absolute left-4 right-4 bottom-4 rounded-xl p-4 bg-white/25 backdrop-blur-md border border-white/30">
      <div className="flex items-center gap-2.5 mb-2.5">
        <img src={avatarPia} alt="Pia Pratt" className="w-9 h-9 rounded-md object-cover" loading="lazy" />
        <div>
          <p className="text-white text-[13px] font-bold leading-tight">Pia Pratt</p>
          <p className="text-white/80 text-[11px] leading-tight mt-0.5">Product Designer at Lightdash</p>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto">
          ...
          <p className="mt-3 text-[13px] md:text-[15px] text-muted-foreground leading-relaxed">
            Fog Bandit isn't theoretical, it's been tested in real break-ins where seconds made the difference
            between loss and protection.
          </p>
        </div>
